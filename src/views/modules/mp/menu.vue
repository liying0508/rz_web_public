...
export default {
  ...
  methods: {
    ...
    getMenu (appId) {
      this.$http.get('/mp/menu/' + appId).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    init () {
      this.$http.get('/mp/account/page').then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
    ...
    getNewsList () {
      ...
      this.$http.get('/mp/material/page', {
        appId: this.appId,
        type: 'news',
    ...
    async onMenuSubmit () {
      ...
      this.$http.post('/mp/menu/' + this.appId, dataForm).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.$message({
    ...
    async onMenuClear () {
      try {
        ...
        this.$http.delete('/mp/menu/' + this.appId).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
