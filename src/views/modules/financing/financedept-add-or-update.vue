  ...
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item label="机构类型" prop="type">
        <ren-radio-group v-model="dataForm.type" dict-type="finance_dept_type"></ren-radio-group>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 获取部门列表
    getDeptList () {
      return this.$http.get('/financing/finance-dept/list').then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/financing/finance-dept/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs.dataForm.validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/financing/finance-dept', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
