...
<!--            <el-select v-model="dataForm.institution" placeholder="金融机构" class="col-input">
      ...
      <finance-credit-reporting-measures :disabled="dataForm.isDisabled" :data-form="dataForm"></finance-credit-reporting-measures>
      <el-row>
...
export default {
  ...
  methods: {
    ...
    async getInfo () {
      try {
        const { data } = await this.$http.get('/financing/fcreditrecordhistory/' + this.dataForm.id + '/' + this.$store.state.unit)
        if (data.code !== 0) {
          return this.$message.error(data.msg)
        }
