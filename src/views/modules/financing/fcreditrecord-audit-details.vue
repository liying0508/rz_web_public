...
export default {
  ...
  methods: {
    ...
    async getInfo () {
      try {
        const { data } = await this.$http.get('financing/recordaudit/' + this.dataForm.id +'/'+this.$store.state.unit)
        if (data.code !== 0) {
          return this.$message.error(data.msg)
        }
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs.dataForm.validate(async (valid) => {
        ...
        const { data } = await this.$http.post('/financing/recordaudit/check', {
          checked: result,
          checkedDes: opinion,
          id: id,
          creditId: id
