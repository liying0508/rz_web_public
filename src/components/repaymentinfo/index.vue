  ...
  <div>
    ...
    <vxe-table border show-overflow :data="payPlanData.payPlan"
      ...
      <vxe-colgroup>
        ...
        <vxe-column field="alreadyRepayPrincipal" :title="`已还本金（${$getDictLabel('unit',$store.state.unit)}）`" :edit-render="{autofocus: '.vxe-input--inner'}">
          <template #edit="{ row }">
            <vxe-input v-model="row.alreadyRepayPrincipal" type="text" @blur="columnChange"></vxe-input>
          </template>
          <template #default="{row}">
            {{row.alreadyRepayPrincipal}}
        ...
        <vxe-column title="操作">
          <template slot-scope="scope">
            <el-button type="text" size="small" :disabled="dataForm.isDisabled" @click="deleteHandler(scope)">{{ $t('delete') }}</el-button>
          </template>
        </vxe-column>
      </vxe-colgroup>
    </vxe-table>
    <vxe-toolbar class="vxe-toolbar-class">
      <template v-slot:buttons>
        <el-button type="primary" size="small" class="add-upload-btn" @click="addRow" :disabled="dataForm.isDisabled">
...
export default {
  ...
  computed: {
    tableHeight () {
      ...
      if (this.payPlanData.payPlan.length === 0) {
        ...
        height = 119 + 48 * this.payPlanData.payPlan.length
      } else {
        height = 311
      }
      return height + 'px'
  ...
  data () {
    return {
      ...
      importUrl: `${window.SITE_CONFIG.apiURL}/financing/excel/importForm?token=${Cookies.get('token')}&templateType=4`
    }
  },
  watch: {
    issueQuota () {
    ...
    dataForm () {
      this.payPlanData.payPlan = this.dataForm.repaymentInfo.repayPrincipalInfoList ? this.dataForm.repaymentInfo.repayPrincipalInfoList : []
    ...
    'payPlanData.payPlan': {
      handler (val) {
        console.log('还本数组改变')
        if (val.length > 0) {
          this.dataForm.repaymentInfo.repayPrincipalInfoList = val
          this.getRepaymentMoney()
        } else {
          this.repaymentMoney = 0
          this.dataForm.repaymentInfo.repayInterestInfoList.forEach(item => {
            this.repaymentMoney += Number(item.alreadyRepayInterest)
          })
        }
      },
      deep: true
    ...
    getRepaymentMoney () {
      ...
      this.dataForm.repaymentInfo.repayPrincipalInfoList.forEach(item => {
        if (this.$store.state.unit == 2) {
          ...
          console.log(repaymentMoney, Number(item.alreadyRepayPrincipal) * 10000)
        } else {
