      ...
      <vxe-colgroup>
        <template #header>
          <el-row class="table-row">
            ...
            <el-col :span="6">
              <span class="item-label">结束日结息：</span>
              <el-switch
            ...
            <el-col :span="7">
              <el-row>
                <el-col :span="7"><span class="item-label">还款金额：</span></el-col>
...
<!--                <el-col :span="17"><money-input :value.sync="repaymentAmount"></money-input></el-col>-->
          ...
          <el-row class="table-row">
            <el-col :span="6">
              <span class="item-label">利率计算方式：</span>
              {{ $getDictLabel("interestRatesWay",rateType) }} {{$isNotNull(rate)&& rateType=='0'?`(${rate} %)`:''}}
            </el-col>
            <el-col :span="6" v-show="$isNotNull(serviceCycle)">
              <span class="item-label">付息周期：</span>  {{$getDictLabel('serviceCycle',serviceCycle)}}
            </el-col>
          ...
          <el-row class="table-row">
            ...
            <el-col :span="6">
              <span class="item-label">累计支付利息：</span>{{ $numberFormat(interestSum )}} {{$getDictLabel('unit',$store.state.unit)}}
            </el-col>
            <el-col :span="6">
              <span class="item-label">累计还款总额：</span>{{  $numberFormat(totalPayment) }} {{$getDictLabel('unit',$store.state.unit)}}
            </el-col>
          </el-row>
        </template>
...
<!--            <vxe-input v-model="row.oddCorpus" type="number"></vxe-input>-->
        ...
        <vxe-column field="desc"  title="说明" v-if="rateType=='1'"></vxe-column>
    ...
    <vxe-toolbar class="vxe-toolbar-class" v-show="isDisabled===false">
      <template v-slot:buttons>
        <el-button type="primary" size="small" class="add-upload-btn" @click="addRow" v-show="isDisabled===false" :disabled="payPlanData.payPlan.length>0&&dataForm.repaymentDate==payPlanData.payPlan[payPlanData.payPlan.length - 1].endDate">
...
export default {
      ...
      handler (val) {
        if (this.$isNotNull(val)) {
          ...
          val.forEach(item => {
            ...
            totalPayment = totalPayment + Number(item.interest ? item.interest : 0) + Number(item.repaymentOfPrincipal ? item.repaymentOfPrincipal : 0)
          })
  ...
  methods: {
    ...
    async testRate () {
      ...
      if (!this.$isNotNull(this.pageType)) {
    ...
    recursionLpr (startTime, endTime, list, lpr) {
      ...
      if (this.$isNotNull(_lpr)) {
        ...
        // console.log('跨周期的：', index, startTime, lpr)
        if (index > -1) {
          _lpr = lpr[index]
    ...
    addRow () {
      ...
      // 自动计算结束日期
      var endDate = ''
      if (!(this.dataForm.serviceCycle == '' || this.dataForm.serviceCycle == 0)) {
        endDate = this.$addMonths(this.dataForm.loanDate, parseInt(this.dataForm.serviceCycle) * (this.payPlanData.payPlan.length + 1))
        if (parseInt(this.dataForm.repaymentDate.split('-').join('')) < parseInt(endDate.split('-').join(''))) {
          endDate = this.dataForm.repaymentDate
        }
      }
      if (this.dataForm.repaymentDate) {
    ...
    deleteHandler (scope) {
      ...
      if (scope.rowIndex != this.payPlanData.payPlan.length - 1) {
        ...
        for (var i = scope.rowIndex + 1; i <= this.payPlanData.payPlan - 1; i++) {
