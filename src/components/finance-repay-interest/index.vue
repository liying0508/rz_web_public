<template>
  <div>
    ...
    <vxe-table border show-overflow :data="payPlanData.payPlan1" v-if="showTable"
...
<!--                <el-col :span="17"><money-input :value.sync="repaymentAmount"></money-input></el-col>-->
        ...
        <vxe-column field="interest" :title="`偿还利息（元）`" :edit-render="{autofocus: '.vxe-input--inner'}">
          <template #edit="{ row }">
            <vxe-input v-model="row.interest" type="text" @blur="columnChange" :disabled="row.planType==1"></vxe-input>
          </template>
        ...
        <vxe-column field="remark"  title="说明" v-show="rateType==1"></vxe-column>
    ...
    <vxe-toolbar class="vxe-toolbar-class" v-show="isDisabled===false">
      <template v-slot:buttons>
        <el-button type="primary" size="small" class="add-upload-btn" @click="addRow" v-show="isDisabled===false" :disabled="payPlanData.payPlan1.length>0&&dataForm.repaymentDate==payPlanData.payPlan1[payPlanData.payPlan1.length - 1].endDate">
...
export default {
  ...
  computed: {
    tableHeight () {
      // 固定表头及动态高度
      var height
      if (this.payPlanData.payPlan1.length === 0) {
        height = 257
      } else if (this.payPlanData.payPlan1.length > 0 && this.payPlanData.payPlan1.length <= 4) {
        height = 209 + 48 * this.payPlanData.payPlan1.length
      } else {
        height = 401
      }
      return height + 'px'
  ...
  data () {
    return {
      ...
      totalPayment: 0,
      showTable: true,
      importUrl: `${window.SITE_CONFIG.apiURL}/financing/excel/importForm?token=${Cookies.get('token')}&templateType=1`
    }
  },
  watch: {
    issueQuota () {
    ...
    'payPlanData.payPlan1': {
      handler (val) {
        console.log('利息数组', val)
  ...
  methods: {
    ...
    successHandle (data) {
      ...
      this.payPlanData.payPlan1 = data
    },
    downloadTemplate () {
    ...
    columnChange (val) {
      ...
      // const list = this.payPlanData.payPlan1
      // this.interestSum = 0
    ...
    disabledStartDateMethod ({ date }) {
      ...
      if (this.$isNotNull(this.valueDate) && this.currentRowIndex === 0) {
        ...
        const valueDate = new Date(this.payPlanData.payPlan1[this.currentRowIndex - 1].endDate).getTime() + 24 * 60 * 60 * 1000
        const startTime = date.getTime()
        return startTime - valueDate + 24 * 60 * 60 * 1000 < 0
      }
      return false
    ...
    disabledDateMethod ({ date }) {
      ...
      const startDate = this.payPlanData.payPlan1[this.currentRowIndex].startDate
    ...
    async testRate () {
      console.log(this.dataForm, this.payPlanData.payPlan1)
      ...
      // 直融、间融、政府债券的利息测算数据从后台获取
      if (this.pageType === 1 || this.pageType === 2 || this.pageType === 3) {
        // 获取dataForm，根据实际接口情况组装接口参数
        var params = {}
        params.interestRatesWay = this.pageType === 1 ? '0' : this.dataForm.interestRatesWay
        params.rate = this.dataForm.rate
        params.lprCycle = this.dataForm.lprCycle
        params.floatRate = this.pageType === 1 || this.pageType === 3 ? 0 : this.dataForm.floatRate
        params.lprInterestCycle = this.dataForm.lprInterestCycle
        params.withdrawalAmount = this.pageType === 1 ? this.dataForm.issueQuota : this.pageType === 2 ? this.dataForm.withdrawalAmount : this.dataForm.financingAmount
        params.principalList = this.dataForm.payPlanData.payPlan
        // planType:1是还本的利息，2是正常利息,此处利息测算应过滤还本的利息
        params.interestList = this.dataForm.payPlanData.payPlan1.filter(item => item.planType !== 1)
        params.interestMeasurementCycle = this.payPlanData.interestMeasurementCycle
        params.endNeedCount = this.payPlanData.endNeedCount ? 1 : 0
        params.isPayPrincipalAndInterest = this.payPlanData.isPayPrincipalAndInterest ? 1 : 0
        params.unit = this.$store.state.unit
        console.log('利息测算接口请求参数', params)
        var url
        if (this.pageType === 1) {
          // 直融利息测算接口
          delete params.lprCycle
          delete params.lprInterestCycle
          url = '/financing/fdirectfinancing/interest'
        } else if (this.pageType === 2) {
          // 间融利息测算接口
          url = '/financing/findirectfinancing/interest'
        } else {
          // 政府债券利息测算接口
          url = '/financing/fgovernmentbond/interest'
        }
        const { data } = await this.$http.post(url, params)
        if (data.code !== 0) {
          return this.$message.error(data.msg)
        }
        this.payPlanData.payPlan1 = data.data
      } else {
        this.testRate1()
      }
    },
    ...
    testRate1 () {
      ...
      let _index = 0
      for (const item of this.payPlanData.payPlan1) {
        // 还款本金
        ...
        if (endTime >= dueDate) {
          ...
          let _item = this.payPlanData.payPlan1[_index]
          if (_index > 0) {
            _item = this.payPlanData.payPlan1[_index - 1]
          }
    ...
    addRow () {
      ...
      if (!this.$isNotNull(this.payPlanData.payPlan1)) {
        this.payPlanData.payPlan1 = []
      }
      let startDate = ''
      if (this.payPlanData.payPlan1.length > 0) {
        if (this.payPlanData.payPlan1[this.payPlanData.payPlan1.length - 1].endDate == '') {
          this.$message.error('请输入还款日期')
          return
        }
        const lastEndDate = new Date(this.payPlanData.payPlan1[this.payPlanData.payPlan1.length - 1].endDate).getTime() + 24 * 60 * 60 * 1000
      ...
      // 自动计算结束日期
      var endDate = ''
      // 还息方式不为空也不为其他时，结束日期=起息日+（还本方式对应值*（还本计划数组长度+1））个月
      if (!(!this.dataForm.interestMethod || this.dataForm.interestMethod == '0')) {
        endDate = this.$addMonths(this.valueDate, parseInt(this.dataForm.interestMethod) * (this.payPlanData.payPlan1.length + 1))
      ...
      if (this.dueDate) {
        this.payPlanData.payPlan1.push(
          {
            startDate: startDate,
            endDate: endDate,
            // 偿还利息
        ...
        console.log(this.payPlanData.payPlan1)
      }
    },
    // 获取还款计划内容
    getPlan () {
      console.log('测试')
      this.payPlanData.payPlan1 = this.payPlanData.payPlan1.filter(item => !this.$isNotNull(item.startDate) || !this.$isNotNull(item.endDate))
      this.$emit('plan', this.payPlanData)
    },
    // 删除当前行
    deleteHandler (scope) {
      if (scope.rowIndex != this.payPlanData.payPlan1.length - 1) {
        // 删除前修改下一周期的开始日期为当前删除周期的开始时期
        this.payPlanData.payPlan1[scope.rowIndex + 1].startDate = this.payPlanData.payPlan1[scope.rowIndex].startDate
      }
      this.payPlanData.payPlan1.splice(scope.rowIndex, 1)
    }
  },
  created () {
    this.getLpr()
